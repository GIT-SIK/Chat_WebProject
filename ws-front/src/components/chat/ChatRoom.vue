<template>
  <v-card class="mx-auto d-sm-flex flex-column cr-card" variant="text">
    ...
    <template v-else>
      <div class="ws-chat-noid">채팅방을 선택해주세요.</div>
    </template>
  </v-card>
</template>
  ...
  setup() {
    ...
    */
    ...
    const scrollAutoDown = () => {
      /* DOM 업데이트 후 함수 실행 */
      nextTick(() => {
        const lct = document.querySelector('.chatdata-list')
        if (!lct) return
...
.cr-card .v-card-actions {
  background-color: #ccd5ae;
  height: 63px;
}

.chat-message-input .v-input__control {
  padding-left: 10px;
  background-color: #ffffff;
  border-radius: 5px;
...
/* 스크롤바 스타일 */
.chatdata-list::-webkit-scrollbar {
  width: 6px;
}

.chatdata-list::-webkit-scrollbar-thumb {
  background: #bbb;
  border-radius: 3px;
}

.chatdata-list::-webkit-scrollbar-thumb:hover {
  background: #999;
}
...
/* 메시지 스타일 총괄 */
.chatdata-list span:nth-child(1) {
  padding: 10px 15px;
  font-size: 14px;
  line-height: 1.4;
  border-radius: 15px;
...
/* 메시지 스타일 시간 */
.chatdata-list span:nth-child(2) {
  font-size: 12px;
}
